...
const footer = () =>{
    ...
    ${`<p>© ${new Date().getFullYear()} ${config.authorName}, Find the code on <a href=${config.authorGithub}>GitHub</a></p>`}
    </footer>`
}

const homePage = (data,post) =>`
<!DOCTYPE html>
<html lang="en">
    <head>
        <meta charset="UTF-8" />
...
const addHomePage = (post) => {
    const data = fs.readFileSync(`${config.dev.homePage}`,'utf-8');
    const content = fm(data);
    content.body = marked.parse(content.body);

    if (!fs.existsSync(`${config.dev.outdir}`)) fs.mkdirSync(`${config.dev.outdir}`);

    fs.writeFile(`${config.dev.outdir}/index.html`,homePage(content,post),
...
module.exports = {createAbout,footer,addPostsPage,addHomePage};